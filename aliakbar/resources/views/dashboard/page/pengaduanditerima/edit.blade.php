    ...
    <div class="row">			  
        <div class="col-lg-10 col-12">
              <div class="box">
                ...
                <!-- /.box-header -->
                <form class="form" method="POST" action="{{ url('dashboard/pengaduan-diterima/update/'.$edit->id)}}" enctype="multipart/form-data">
                    @csrf @method('PATCH')
                    <div class="box-body">
                      <div class="row">
                         <div class="col-md-8">
                        ...
                        <div class="form-group @error('status') error @enderror">
                          ...
                          <select name="status" class="form-control select" style="width: 100%;">
                            ...
                            @foreach($status as $status)
                                <option @if(old('status',$edit->status_id) == $status->id) selected="" @endif value="{{ $status->id }}">{{ $status->nama }}</option>
                            @endforeach
                          </select>
                          @error('status')
                          <div class="help-block"><ul role="alert"><li>{{$message}}</li></ul></div>
  ...
  <script>  
    $(document).ready(function() {
      ...
      var tabel = $('.tabel').DataTable({
           ...
           serverSide: true,
           ajax: {
           url: "{{ url('dashboard/pengaduan-diterima/table-file') }}/{{ $edit->id }}",
           },
           columns: [
